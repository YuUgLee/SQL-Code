...
--Issue the statement select count(*) from <your table name here>
...
--INNER JOINS----------
-- List the customer name of any customer located in a different state from the office where their sales representative works.
-- Many countries do not have states. The value for addresses in those countries will have a null for the state.
-- You need to use the coalesce function to allow you to compare those null values to other non-null values.
-- Please consult the documentation for coalesce here.  Also, state names are only unique within their country.
-- Two states could have the same name but be in different countries.  List the customers in ascending order by customer name. (58)
SELECT customername
   FROM customers INNER JOIN employees ON customers.salesrepemployeenumber = employees.employeenumber
                  INNER JOIN offices ON employees.officecode = offices.officecode
   WHERE customers.state <> COALESCE(offices.state, 'N/A') OR customers.country <> offices.country
   ORDER BY customername ASC;

-- List the customers who have ever purchased the ‘Pont Yacht’ product.
-- Be sure to list each customer only once.  Order by customer name. (24)
-- USE DISTINCT
...
-- List the city, state, and country of each office that has an employee working at that office and the employee
-- is the sales representative for a customer who has ever bought ‘1928 Mercedes-Benz SSK’. (5)
SELECT DISTINCT offices.city, COALESCE(offices.state, 'N/A'), offices.country
   FROM offices INNER JOIN employees ON offices.officecode = employees.officecode
                INNER JOIN customers ON employees.employeenumber = customers.SALESREPEMPLOYEENUMBER
                INNER JOIN orders ON customers.customernumber = orders.CUSTOMERNUMBER
                INNER JOIN orderdetails ON orders.ordernumber = orderdetails.ORDERNUMBER
                INNER JOIN products ON orderdetails.productcode = products.PRODUCTCODE
   WHERE productname = '1928 Mercedes-Benz SSK';

-- Select customer name, order date, shipped date, quantity ordered, product line,
-- product name for all orders made in May of 2013 and shipped in some other month than May of 2013.
-- Order by customer name and order date. (17)
SELECT customers.customername, orders.orderdate, orders.shippeddate, orderdetails.quantityordered, products.productline, products.PRODUCTNAME
   FROM customers INNER JOIN orders ON customers.customernumber = orders.CUSTOMERNUMBER
                  INNER JOIN orderdetails ON orders.ordernumber = orderdetails.ORDERNUMBER
                  INNER JOIN products ON orderdetails.productcode = products.PRODUCTCODE
   WHERE orders.orderdate BETWEEN '2013-05-01' AND '2013-05-31' AND orders.shippeddate > '2013-05-31'
   ORDER BY customername ASC, orderdate ASC;

-- Select customer name, order number, order date, and status on all orders in which at least one of the products
-- ordered has a quantity ordered > the quantity in stock for that product.  List each order no more than once.
-- Order by the customer name and the order number. (28)
SELECT customers.customername, orders.ordernumber, orders.orderdate, orders.status
   FROM customers INNER JOIN orders ON customers.customernumber = orders.CUSTOMERNUMBER
                  INNER JOIN orderdetails ON orders.ordernumber = orderdetails.ORDERNUMBER
                  INNER JOIN products ON orderdetails.productcode = products.PRODUCTCODE
   WHERE orderdetails.quantityordered > products.quantityinstock
   ORDER BY customers.customerName, orders.orderNumber;

--Set operations
-- 1. List all customers who didn't order in 2015.  List them in ascending order by customer name.  (78)
...
-- 2. List all people that we deal with (employees and customer contacts). Display first name, last name, company name (or employee) order by last name, then first name.  (145)
SELECT customers.contactFirstName AS fName, customers.contactLastName AS lName, customers.customerName
    FROM customers
UNION
SELECT employees.firstName AS fName, employees.lastName AS lName, employees.officeCode
    FROM employees
    ORDER BY lName, fName;

-- 3. List the last name, first name, and employee number of all of the employees who do not have any customers.  Order by last name first, then the first name. (8).
SELECT employees.lastName as lName, employees.firstName as fName, employees.employeeNumber
from employees
EXCEPT
SELECT employees.lastName as lName, employees.firstName as fName, employees.employeeNumber
from employees where employees.employeeNumber in (SELECT customers.salesRepEmployeeNumber from customers)
order by lName, fName;

-- 4. List the states and the country that the state is part of that have customers but not offices, offices but not customers, or both one or more customers and one or more offices all in one query.  Designate which state is which with the string 'Customer', 'Office', or 'Both'.  If a state falls into the “Both” category, do not list it as a Customer or an Office state.  Order by the country, then the state.  Give the category column (where you list ‘Customer’, ‘Office’, or ‘Both’) a header of “Category” and exclude any entries in which the state is null. (19)
(SELECT customers.state as state, customers.country as country, 'Customer' as Category
from customers
where state is not null
except
SELECT customers.state as state, customers.country as country, 'Customer' as Category
from customers
where customers.state in (SELECT offices.state from offices))
union
(SELECT offices.state as state, offices.country as country, 'Office' as Category
from offices
where state is not null
except
SELECT offices.state as state, offices.country as country, 'Office' as Category
from offices
where offices.state in (SELECT customers.state from customers))
union
SELECT customers.state as state, customers.country as country, 'Both' as Category
from customers inner join offices on customers.state = offices.state
where customers.country = offices.country
order by country, state;

-- 5. List the Product Code and Product name of every product that has never been in an order in which the customer asked for more than 48 of them.  Order by the Product Name.  (8)
SELECT products.productCode, products.productName as prodName
from products
except
SELECT products.productCode, products.productName as prodName
...
-- 6. List the first name and last name of any customer who ordered any products from either of the two product lines ‘Trains’ or ‘Trucks and Buses’.  Do not use an “or”.  Instead perform a union.  Order by the customer’s name.  List the customers in ascending order by customer name.  (61)
SELECT DISTINCT contactlastname, contactfirstname, customerName
...
-- 7. List the name of all customers who do not live in the same state and country with any other customer.  Do not use a count for this exercise.  Order by the customer name.  (17)
(SELECT c1.customerName as custName, coalesce(c1.state,'N/A'), c1.country
from CUSTOMERS as c1
except
(SELECT c1.customerName as custName, coalesce(c1.state,'N/A'), c1.country
from CUSTOMERS as c1
where c1.country in (SELECT c2.country from customers as c2 where c2.customerName != c1.customerName)))
union
(SELECT c1.customerName as custName, coalesce(c1.state,'N/A'), c1.country
from CUSTOMERS as c1
where c1.country in (SELECT c2.country from customers as c2 where c2.customerName != c1.customerName)
...
(SELECT c1.customerName as custName, coalesce(c1.state,'N/A'), c1.country
from CUSTOMERS as c1
where coalesce(c1.state,'N/A') in (SELECT coalesce(c2.state,'N/A') from customers as c2 where c2.customerName != c1.customerName and c2.country = c1.country)))
order by custName;

--Recursion
--#1
-- Report out the customer number of every customer who has never made a payment on the same date as another customer.
-- Note, you will need the except for this.  Order by customer number.  Returns 33 rows
SELECT DISTINCT c1.CUSTOMERNUMBER
   FROM payments c1
EXCEPT
SELECT c2.CUSTOMERNUMBER
   FROM customers c2 INNER JOIN payments p1 ON p1.customernumber = c2.customernumber
                     INNER JOIN payments p2 ON p2.paymentdate = p1.paymentdate
   WHERE p1.customernumber <> p2.customernumber
   ORDER BY customernumber;

--#2
-- Report the employee number, last name, and first name of every employee who is not a manager.
-- Remember that the reportsTo field in employee carries the employee number of the manager of that employee.
-- Sort by last name, then first name.  Returns 17 rows.
SELECT e.employeeNumber AS emplNumber, e.lastName AS lastName, e.firstName AS firstName
FROM employees e
EXCEPT
SELECT e.employeeNumber emplNumber, e.lastName AS lastName, e.firstName AS firstName
FROM employees e INNER JOIN employees man ON man.reportsTo = e.employeeNumber
ORDER BY lastName,firstName;



SELECT DISTINCT e.employeeNumber emplNumber, count(e.employeeNumber)
FROM employees e INNER JOIN employees man ON man.reportsTo = e.employeeNumber
...
--#3
-- List the first customer name, their city, state, country, and the second customer name for every pair of customers
-- who share the same city, state, country, and sales rep.  Remember that the customer table has the
-- salesRepEmployeeNumber which is the surrogate for the Employee who serves as that customer’s sales representative.
-- Also remember that the state may be null, so frame your join accordingly.  Be sure to show a given pair of customers
-- only once.  For instance, I do not want to see one row with American Souvenirs Inc and Super Scale Inc.  and then,
-- later, a row that is Super Scale Inc. and American Souvenirs Inc.  Order by the first customer name, then the second.
-- Returns 18 rows.
SELECT c1.customername, c2.customername, c1.city, COALESCE(c1.state,'N/A'), c1.country
   FROM customers c1 INNER JOIN customers c2 ON c1.salesrepemployeenumber = c2.salesrepemployeenumber
   WHERE c1.salesrepemployeenumber = c2.salesrepemployeenumber AND c1.city = c2.city AND c1.country = c2.COUNTRY
           AND c1.customername < c2.customername
   ORDER BY c1.customername, c2.customername;

--#4
-- Report out the last name, the first name, and the type of employee for three classes of employee: those who don’t
-- report to anyone, those who report to anyone who doesn’t report to anyone, and those who report to those who report
-- to anyone who doesn’t report to anyone.  The type of employee will be ‘Big Kahuna’ in the first case,
-- ‘1st Level Down’ in the second case, and ‘2nd Level Down’ in the third case.  Order by the last name,
-- then the first name.  Returns 7 rows: 1 in the first type, 2 in the second, and 4 in the third type.
-- Broad hint, you are going to have to do two unions for this.
SELECT employees.lastName AS lastName, employees.firstName AS firstName, 'Big Kahuna' AS typeOfEmployee
FROM employees
WHERE employees.reportsTo IS NULL
UNION
SELECT e.lastName AS lastName, e.firstName AS firstName, '1st Level Down' AS typeOfEmployee
FROM employees e INNER JOIN employees man ON e.reportsTo = man.employeeNumber
WHERE man.reportsTo IS NULL
UNION
SELECT e.lastName AS lastName, e.firstName AS firstName, '2st Level Down' AS typeOfEmployee
FROM employees e INNER JOIN employees man ON e.reportsTo = man.employeeNumber INNER JOIN employees bigKahuna ON man.reportsTo = bigKahuna.employeeNumber
WHERE bigKahuna.reportsTo IS NULL
ORDER BY lastName, firstName;

--#5
-- For all pairs of orders that share the same order date, shipped date, and status, report the order numbers, their
-- order date, shipped data and status.  Be careful not to show a given pair of orders twice.  Order by the first order
-- number, then the second.  Returns 15 rows.
SELECT o1.ordernumber, o2.ordernumber, o1.orderdate, o1.shippeddate, o1.status
   FROM orders o1 INNER JOIN orders o2 ON o1.orderdate = o2.ORDERDATE
   WHERE o1.orderdate = o2.orderdate AND o1.shippeddate = o2.shippeddate AND o1.status = o2.STATUS AND o1.ordernumber<o2.ORDERNUMBER
   ORDER BY o1.ordernumber, o2.ordernumber;
...
SELECT customerName, contactfirstname, contactlastname, checknumber, amount
    ...
    WHERE amount > 77777.77
    ORDER BY amount DESC
;
SELECT customername, orderDate, quantityOrdered, productname
    FROM customers INNER JOIN orders ON customers.customernumber = orders.CUSTOMERNUMBER
                INNER JOIN orderdetails ON orders.ordernumber = orderdetails.ORDERNUMBER
                INNER JOIN products ON orderdetails.productcode = products.productcode
    WHERE productname='1957 Ford Thunderbird' OR productname='1952 Citroen-15CV'
    ORDER BY customername, productname
;

SELECT e.firstname, e.lastname, e.employeeNumber, c1.customername, c2.customername, c1.city, c1.country
  FROM customers c1 INNER JOIN customers c2 ON c1.salesrepemployeenumber = c2.salesrepemployeenumber
                    INNER JOIN employees e on c2.salesRepEmployeeNumber = e.employeeNumber
  WHERE c1.salesrepemployeenumber = c2.salesrepemployeenumber AND c1.city = c2.city AND c1.country = c2.COUNTRY
          AND c1.customername < c2.customername
  ORDER BY c1.customername, c2.customername;
...
SELECT (CASE WHEN MONTH(orders.orderdate) IN (1, 2, 3) THEN 'WINTER'
                WHEN MONTH(orders.orderdate) IN (4, 5, 6) THEN 'SPRING'
                WHEN MONTH(orders.orderdate) IN (7, 8, 9) THEN 'SUMMER'
                WHEN MONTH(orders.orderdate) IN (10, 11, 12) THEN 'AUTUMN'
           END) as season, customers.customername, orders.orderdate
   FROM customers INNER JOIN orders ON customers.customernumber = orders.CUSTOMERNUMBER
                  INNER JOIN orderdetails ON orders.ordernumber = orderdetails.ORDERNUMBER
                  INNER JOIN products ON orderdetails.productcode = products.PRODUCTCODE
   WHERE YEAR(orders.orderdate) = 2015 AND customername LIKE '%Collect%'
...
-- #1 List out the customer name, order number, and the order date for all orders where to total of the order is greater
-- than 90% of the largest order.  The order total is the sum of the quantity ordered * the price each for all the order
-- details within a given order.  Remember that when you perform a group by you can only select out aggregate function
-- values, or columns that were in the group by.  Also remember that adding a column like customer name to a group by
-- that already has order number will not change the result since for each customer there are several orders.
-- Returns 3 rows.

SELECT customername, o.ordernumber, orderdate, SUM(quantityordered *priceEach) as Total
...
HAVING SUM(quantityordered *priceEach) >(0.90 *(SELECT MAX (total) FROM
(SELECT customername, o.ordernumber, orderdate, SUM(quantityordered *priceEach) as Total
    FROM customers c INNER JOIN orders o ON c.customerNumber = o.customerNumber
                   INNER JOIN orderdetails od ON o.orderNumber = od.orderNumber
GROUP BY customername, o.ordernumber, orderdate) as t1))
;

SELECT MAX (totalamount) FROM
(SELECT customername, o.ordernumber, orderdate, SUM(quantityordered *priceEach) as totalamount
    FROM customers c INNER JOIN orders o ON c.customerNumber = o.customerNumber
                   INNER JOIN orderdetails od ON o.orderNumber = od.orderNumber
GROUP BY customername, o.ordernumber, orderdate) as t1
;
...
-- #2 For each office, report out the office code, city, state, country of each of the customers who’s sales representative
-- works in that office.  Only report city, state, and country for a given customer if there are more than one customer
-- in that city, state, and  country served by a sales representative from that office.  Be careful of the null states
-- in your group by.  Order by the office code, the city, state, and country of the customers.  Returns 12 rows.
SELECT  offices.officecode,customers.city ,COALESCE(customers.state,'N/A') AS state,customers.country
    FROM offices INNER JOIN employees ON offices.officecode = employees.officecode
                  INNER JOIN customers ON employees.employeenumber = customers.salesrepemployeenumber
GROUP BY customers.country,customers.state,customers.city, offices.officecode
HAVING COUNT(customers.city)>1
;

-- #4 For all the orders that were placed the same day as our very first order, list the order number, order date, the
-- customer name, and the number of days between the order date and the shipped date.  Derby does not provide a SQL
-- function for date arithmetic per se.  But it does provide a function interface.
-- The SQL: select … {fn timestampdiff(SQL_TSI_DAY, <date expression 1>, <date expression 2>)} … will return the number
-- of days between the dates represented by date expression 1 and date expression 2.  Returns 1 row, but do not assume
-- that in your code.
SELECT orders.ordernumber, orders.orderdate, customers.customername, {fn timestampdiff(SQL_TSI_DAY, orders.orderdate, orders.shippeddate)} as Days_Between_OrderDate_ShippedDate
   FROM orders INNER JOIN customers ON customers.customernumber = orders.CUSTOMERNUMBER
GROUP BY orders.ordernumber, orders.orderdate, customers.customername, orders.shippeddate
HAVING orders.orderdate=(SELECT MIN (orders.orderdate)FROM orders)
;

SELECT customers.customername, p1.productname, orders.ordernumber, orders.orderdate, (od1.quantityOrdered*od1.priceEach) as subtotal
    FROM customers INNER JOIN orders USING (customerNumber)
                   INNER JOIN orderdetails as od1 USING (orderNumber)
                   INNER JOIN products as p1 USING (productCode)
WHERE (od1.quantityOrdered*od1.priceEach) >= ALL
(SELECT (od2.quantityOrdered*od2.priceEach) as subtotal
    FROM customers INNER JOIN orders USING (customerNumber)
                   INNER JOIN orderdetails as od2 USING (orderNumber)
                   INNER JOIN products as p2 USING (productCode)
...
SELECT customers.customername, p1.productname, orders.ordernumber, orders.orderdate, (od1.quantityOrdered*od1.priceEach) as subtotal
    FROM customers INNER JOIN orders USING (customerNumber)
                   INNER JOIN orderdetails as od1 USING (orderNumber)
                   INNER JOIN products as p1 USING (productCode)
WHERE NOT EXISTS (SELECT  (od2.quantityOrdered*od2.priceEach)
                 FROM orderdetails as od2
                    INNER JOIN products as p2 on p2.productCode = od2.productCode
                 WHERE p1.productCode = p2.productCode AND od1.quantityOrdered*od1.priceEach < od2.quantityOrdered*od2.priceEach)
ORDER BY subtotal DESC
;

SELECT p3.productline, p3.productname
FROM products AS p3
WHERE p3.productline = (SELECT p1.productline
   FROM products AS p1
   GROUP BY p1.productline
   HAVING COUNT(p1.productname) >= ALL (
       SELECT COUNT(p2.productname) AS Num_Products
       FROM products AS p2
       GROUP BY p2.productline))
ORDER BY p3.productline, p3.productname
;
SELECT p1.productLine,p1.productName
from products as p1
where not exists (SELECT p3.productLine
                  from products p3
                  group by p3.productLine
                  having count(p3.productName) > (SELECT count(p2.productName)
       from products as p2
        where p1.productLine = p2.productLine));

SELECT p1.productname, p2.productname, p3.productname
FROM (SELECT productname, ordernumber, productcode FROM products INNER JOIN orderdetails USING (productcode)) AS p1 INNER JOIN
    (SELECT productname, ordernumber, productcode FROM products INNER JOIN orderdetails USING (productcode)) AS p2  USING (ordernumber) INNER JOIN
    (SELECT productname, ordernumber,productcode FROM products INNER JOIN orderdetails USING (productcode)) AS p3  USING (ordernumber)
WHERE p1.productcode < p2. productcode AND p2.productCode < p3.productcode
GROUP BY p1.productName, p2.productname, p3.productname
...
-- Find all the pairs of employees on whose respective managers have the same last name but those managers are not the same person.
-- List each pair of employees only once (9)
-- As a view
CREATE VIEW turkey as
SELECT e1.lastname as "employee last name", e1.firstname as "employee first name",
       e2.lastname as "manager last name", e2.firstname as "manager first name",
       e1.employeenumber as "emp no", e2.employeeNumber as "man no"
FROM employees e1 INNER JOIN employees e2 ON e1.reportsTo = e2.employeeNumber;

SELECT one."employee last name" as "first emp last name",
       one."employee first name" as "first emp first name",
       theOther."employee last name" as "second emp last name",
       theOther."employee first name" as "second emp first name",
       one."manager last name", one."manager first name" as "first emp manager first name",
       theOther."manager first name" as "second emp manager first name"

FROM
       turkey one
       INNER JOIN
       turkey theOther
       USING ("manager last name")
WHERE  one."man no" < theOther."man no" AND
       one."emp no" < theOther."emp no"
;
...
FROM
    (SELECT e1.lastname as "employee last name", e1.firstname as "employee first name",
            e2.lastname as "manager last name", e2.firstname as "manager first name",
            e1.employeenumber as "emp no", e2.employeeNumber as "man no"
    FROM employees e1 INNER JOIN employees e2 ON e1.reportsTo = e2.employeeNumber)one
    INNER JOIN
    (SELECT e1.lastname as "employee last name", e1.firstname as "employee first name",
            e2.lastname as "manager last name", e2.firstname as "manager first name",
            e1.employeenumber as "emp no", e2.employeeNumber as "man no"
    FROM employees e1 INNER JOIN employees e2 ON e1.reportsTo = e2.employeeNumber)theOther
    USING ("manager last name")
WHERE one."man no" < theOther."man no" AND
      one."emp no" < theOther."emp no"
;

-- Report the order number, customer name, and each of the product lines that are represented in that order.
-- Only show those orders which have the largest number of product lines. Remember you will often have a single
-- order that has multiple products from the same line. Order by the order number and the product line. (15)
...
HAVING Count(ordernumber) > 17)t1
INNER JOIN
(SELECT customerName, COUNT(ordernumber) as "numorders"
FROM customers c INNER JOIN orders o USING (customerNumber)
                 INNER JOIN orderDetails od USING (orderNumber)
                 INNER JOIN products p using (productCode)
GROUP BY  ordernumber, customerName
HAVING Count(ordernumber) > 17)t2
using (customername)

;
SELECT  t1.customername as "one customer", t2.customername as " the other customer",
        t1.ordernumber as "one order number", t2.ordernumber as "the other order number",
        t1.productname as "product ordered"
FROM    test1 t1
        INNER JOIN
        test1 t2
        USING (productname)
WHERE t1.ordernumber< t2.ordernumber AND t1.productname = t2.productname;
;

HAVING COUNT(p.productname) > 6 AND

      o.ordernumber IN (
SELECT ordernumber
FROM
(SELECT orderNumber, COUNT(DISTINCT productline) as "# product lines"
FROM products p INNER JOIN orderDetails od USING (productCode)
GROUP BY od.orderNumber) lineCnt
WHERE "# product lines" = 6);

SELECT Distinct customerName, ordernumber, productLine
FROM customers c INNER JOIN orders o USING (customerNumber)
                 INNER JOIN orderDetails od USING (orderNumber)
                 INNER JOIN products p using (productCode)
...
GROUP BY od.orderNumber) lineCnt
WHERE "# product lines" =
(SELECT MAX("# product lines")
FROM
(SELECT orderNumber, COUNT(DISTINCT productline) as "# product lines"
FROM products p INNER JOIN orderDetails od USING (productCode)
GROUP BY od.orderNumber) as linecount))
...
-- Find the pairs of orders and the customer who placed each order for those orders that have the same
-- number of items. For instance, if there are two orders, and one is for a quantity of 4 for one model
-- and a quantity of 3 for another model, then a DIFFERENT order with a quantity of 7 for just one item,
-- those two orders would up in the final report. (94)

CREATE VIEW turkey1 AS
SELECT customername, Count(orderNumber) as numItems
FROM orderdetails    INNER JOIN orders o using (orderNumber)
...
FROM orderdetails    INNER JOIN orders o using (orderNumber)
                     INNER JOIN customers  c using (customerNumber)
                     INNER JOIN products p using (productCode)
GROUP BY orderNumber, customerName, productname
WHERE Count(orderNumber) = ALL
(SELECT customername,Count(orderNumber) as numItems
FROM orderdetails    INNER JOIN orders o using (orderNumber)
                     INNER JOIN customers  c using (customerNumber)
                     INNER JOIN products p using (productCode)
...
;
SELECT  t1.customername as "one customer", t2.customername as " the other customer",
        t1.numItems as "the other order number"
FROM    turkey1 t1
        INNER JOIN
        turkey1 t2
        USING (numItems)
WHERE t1.numItems = t2.numItems AND t1.productCode < t2.productCode;

SELECT  t1.customername as "one customer", t2.customername as " the other customer",
        t1.ordernumber as "one order number", t2.ordernumber as "the other order number",
        t1.numItems as "number of items ordered"
FROM
(SELECT customername, orderNumber, sum(quantityOrdered) numItems
FROM orderdetails    INNER JOIN orders using (orderNumber)
                     INNER JOIN customers using (customerNumber)
GROUP BY orderNumber, customerName) t1
INNER JOIN
(SELECT customername, orderNumber, sum(quantityOrdered) numItems
FROM orderdetails    INNER JOIN orders using (orderNumber)
                     INNER JOIN customers using (customerNumber)
GROUP BY orderNumber, customerName) t2
USING (numItems)
WHERE t1.ordernumber< t2.ordernumber AND t1.numItems = t2.numItems
;


-- Find all the products that have NEVER been ordered from a customer whose sales representative
-- is based in the same state with our Boston offices. (4)
SELECT productname
FROM products
EXCEPT
     ;
--NUMBER 3
SELECT DISTINCT t1.customername, t1.country, t1.state, t1.city,t2.customername as "other customer", t2.country, t2.state, t2.city, t1.salesRepEmployeeNumber, t2.salesRepEmployeeNumber

    FROM
(SELECT c.customername, c.country, c.state, c.city, c.salesRepEmployeeNumber
FROM products INNER JOIN orderDetails using (productCode)
              INNER JOIN orders using (orderNumber)
              INNER JOIN customers c using (customerNumber)
              INNER JOIN employees e on c.salesRepEmployeeNumber = e.employeeNumber
              INNER JOIN offices o using (officeCode)) t1
INNER JOIN
(SELECT c.customername, c.country, c.state, c.city, c.salesRepEmployeeNumber
FROM products INNER JOIN orderDetails using (productCode)
              INNER JOIN orders using (orderNumber)
              INNER JOIN customers c using (customerNumber)
              INNER JOIN employees e on c.salesRepEmployeeNumber = e.employeeNumber
              INNER JOIN offices o using (officeCode)) t2
USING (country)
WHERE t1.salesRepEmployeeNumber < t2.salesRepEmployeeNumber AND t1.city=t2.city AND t1.state=t2.state AND t1.customername< t2.customername
;
